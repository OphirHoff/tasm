...
offsetsDifference = 18
STACK 100h
DATASEG
; --------------------------
playerName    db    'xx123456x'
knownPlayer   db    0
WelcomeBackM  db    'Welcome Back! $'
YourScoreM    db    'Your Score is: $'
oldScore      db    5 dup ('$')
score         db    0
EndGame       db    0
life          db    3
...
; Every fruit has it's own "API" with "properties"
		      ...
		      ;X  ;Y
pri1 dw 0, 0, 96, 160, 0, 0, 60, 96, 0
       ; 0. Is Active?
	   ...
	   ; 9. Slice Effect Delay
		      ...
		      ;X   ;Y
pri2 dw 0, 0, 191, 160, 0, 0, 155, 191, 0
       ; 0. Is Active?
	   ; 1. Where is it moving? (up = 0 / down = 1)
	   ; 2. X position
	   ; 3. Y position
	   ...
	   ; 8. Was is used in current 'round'?
		      ...
		      ; X   ;Y
pri3 dw 0, 0, 281, 160, 0, 0, 245, 281, 0
	   ; 0. Is Active?
	   ; 1. Where is it moving? (up = 0 / down = 1)
	   ; 2. X position
	   ; 3. Y position
...
proc Get_Name
	...
	mov ah, 9
	mov dx, offset WelcomeBackM
	int 21h
	
	mov ah, 9
	mov dx, offset YourScoreM
	int 21h
...
proc FindName
	...
	cmp [ReadName+si], 13
	je KeepLooking
	cmp [ReadName+si], ':'  ; If char is 10 (New Line) - stop reading
	je @@Cont
	inc si
	inc dx
	jmp @@Read
